...
module.exports.run = async (client, message, args, embed) => {
  ...
  const xen4 = new discord.MessageEmbed().setDescription(`${message.author} Kicklemek istediğin kişi senden üst yada aynı pozisyonda.`).setColor("RANDOM").setFooter(config.embed.embedFooter).setAuthor(config.embed.sunucuAdı).setTimestamp()
  const xen5 = new discord.MessageEmbed().setDescription(`${message.author} sunucu sahibini kickleyemezsin`).setColor("RANDOM").setFooter(config.embed.embedFooter).setAuthor(config.embed.sunucuAdı).setTimestamp()
  ...
  try {
    const istek = new Discord.MessageEmbed().setAuthor(message.author.username, message.author.avatarURL({ dynamic: true })).setTimestamp().setFooter(config.embed.embedFooter).setColor('RED').setDescription(`<@!${user.id}> Adlı kullanıcı ${message.author} tarafından sunucudan atılacaktı fakat reddedildi.`);
    const logChannel = await message.guild.channels.cache.get(config.logs.kicklog);
    const a = new Discord.MessageEmbed().setAuthor(message.author.username, message.author.avatarURL({ dynamic: true })).setTimestamp().setFooter(config.embed.embedFooter).setColor('GREEN').setDescription(`${config.emojis.yes} <@!${user.id}> Adlı kullanıcı <@!${message.author.id}> tarafından sunucudan atıldı.`);
    const oldembed = new Discord.MessageEmbed().setColor('RED').setDescription(`${message.author}, ${user} Adlı kullanıcıyı sunucudan atmak istediğinize eminmisiniz?`)
    const newEmbeds = new Discord.MessageEmbed().setColor('GREEN').setDescription(`${config.emojis.yes} ${user} Adlı kullanıcı <@!${message.author.id}> tarafından sunucudan atıldı.`)
    ...
    const hayir = new Discord.MessageEmbed().setColor('RED').setDescription(`${config.emojis.no} ${user} Adlı kullanıcıyı ${message.author} tarafından sunucudan atılma reddedildi.`)
    collector.on("collect", async (button) => {
      if (button.id === '2') {
        if (message.member.id != button.clicker.member.id) return button.reply.send(`Komutu sadece ${message.author} yani komutu kullanan kişi kullanabilir.`, true)
        await button.reply.defer()
